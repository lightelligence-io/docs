...
const titleDisplay = $('.scroll-nav-wrapper .scroll-nav .mobile');
const hamburgerIcon = $('#hamburg-icon');
var mobileNavVisible = () => {
  return hamburgerIcon.is(':visible');
};
...
$(window).scroll(function(){
  ...
  var first = false;
  $('h1.lh-title, div#prose > h2').each( function() {
    var offset = $(this).offset();
    if ((scrollTop - 50) <= offset.top && ($(this).height() + offset.top) < (scrollTop + windowHeight) && first == false) {
      $(this).addClass("first");
      // console.log('this', $(this).text(), titleDisplay);
      titleDisplay.text($(this).text());
      first=true;
    }
  });
});
